"""API do microserviço insights — exposto via API Gateway -> VPC Link -> NLB -> EKS.

Analytics read-only multi-produto AWS. Endpoint único `POST /insights/execute`
despacha por `params.action`. Modo de dados via env `INSIGHTS_MODE` (mock|aws).
"""
...
from .models import ActionResult, ErrorResponse, InsightsRequest

app = FastAPI(title="insights action microservice", version="1.0.0")
