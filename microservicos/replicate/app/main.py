"""FastAPI app do serviço `replicate` (POST /replicate/execute)."""
...
from .models import ActionAccepted, ErrorResponse, ReplicateRequest

app = FastAPI(title="replicate action microservice", version="1.0.0")
...
@app.post("/replicate/execute", response_model=ActionAccepted, status_code=202,
          responses={c: {"model": ErrorResponse} for c in (400, 403, 404, 409, 422, 500, 502)})
def run(req: ReplicateRequest) -> ActionAccepted | JSONResponse:
    try:
        return execute(req)
