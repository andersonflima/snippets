# vpc-link — microserviço action-driven

Cria acesso privado (PrivateLink) da conta do time ao banco.
...
`POST /vpc-link/execute` — executa a ação. Health: `GET /healthz`, `GET /readyz`.
...
```bash
docker build -t vpc-link .
docker run -p 8080:8080 vpc-link
```
