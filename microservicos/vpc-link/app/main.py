"""API do microserviço vpc-link — exposto via API Gateway -> VPC Link -> NLB -> EKS."""
...
from .models import ActionAccepted, ErrorResponse, VpcLinkRequest

app = FastAPI(title="vpc-link action microservice", version="1.0.0")
...
def run(req: VpcLinkRequest):
    try:
        ensure_change_authorized("vpc-link", req)
        return execute(req)
