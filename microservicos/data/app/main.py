"""FastAPI app do serviço `data` (POST /data/execute)."""
...
from .models import ActionAccepted, ErrorResponse, DataRequest

app = FastAPI(title="data action microservice", version="1.0.0")
...
@app.post("/data/execute", response_model=ActionAccepted, status_code=202,
          responses={c: {"model": ErrorResponse} for c in (400, 403, 404, 409, 422, 500, 502)})
def run(req: DataRequest) -> ActionAccepted | JSONResponse:
    try:
        return execute(req)
