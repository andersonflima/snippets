"""API do microserviço kms — exposto via API Gateway -> VPC Link -> NLB -> EKS."""
...
from .models import ActionAccepted, ErrorResponse, KmsRequest

app = FastAPI(title="kms action microservice", version="1.0.0")
...
def run(req: KmsRequest):
    try:
        ensure_change_authorized("kms", req)
        return execute(req)
