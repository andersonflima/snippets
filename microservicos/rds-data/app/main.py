"""API do microserviço rds-data — exposto via API Gateway -> VPC Link -> NLB -> EKS."""
...
from .models import ActionAccepted, ErrorResponse, RdsDataRequest

app = FastAPI(title="rds-data action microservice", version="1.0.0")
...
def run(req: RdsDataRequest):
    try:
        ensure_change_authorized("rds-data", req)
        return execute(req)
