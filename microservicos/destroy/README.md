# destroy — microserviço action-driven

Remove recursos (cleanup pós-fluxo).
...
`POST /destroy/execute` — executa a ação. Health: `GET /healthz`, `GET /readyz`.
...
```bash
docker build -t destroy .
docker run -p 8080:8080 destroy
```
