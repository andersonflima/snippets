"""FastAPI app do serviço `destroy` (POST /destroy/execute)."""
...
@app.post("/destroy/execute", response_model=ActionAccepted, status_code=202,
          responses={c: {"model": ErrorResponse} for c in (400, 403, 404, 409, 422, 500, 502)})
def run(req: DestroyRequest) -> ActionAccepted | JSONResponse:
    try:
        return execute(req)
