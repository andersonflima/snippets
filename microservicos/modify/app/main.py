"""FastAPI app do serviço `modify` (POST /modify/execute)."""
...
from .models import ActionAccepted, ErrorResponse, ModifyRequest

app = FastAPI(title="modify action microservice", version="1.0.0")
...
@app.post("/modify/execute", response_model=ActionAccepted, status_code=202,
          responses={c: {"model": ErrorResponse} for c in (400, 403, 404, 409, 422, 500, 502)})
def run(req: ModifyRequest) -> ActionAccepted | JSONResponse:
    try:
        return execute(req)
