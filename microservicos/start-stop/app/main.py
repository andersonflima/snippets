"""FastAPI app do serviço `start-stop` (POST /start-stop/execute)."""
...
from .models import ActionAccepted, ErrorResponse, StartStopRequest

app = FastAPI(title="start-stop action microservice", version="1.0.0")
...
@app.post("/start-stop/execute", response_model=ActionAccepted, status_code=202,
          responses={c: {"model": ErrorResponse} for c in (400, 403, 404, 409, 422, 500, 502)})
def run(req: StartStopRequest) -> ActionAccepted | JSONResponse:
    try:
        return execute(req)
