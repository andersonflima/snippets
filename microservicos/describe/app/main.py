"""FastAPI app do serviço `describe` (POST /describe/execute)."""
...
from .models import ActionAccepted, ErrorResponse, DescribeRequest

app = FastAPI(title="describe action microservice", version="1.0.0")
...
@app.post("/describe/execute", response_model=ActionAccepted, status_code=202,
          responses={c: {"model": ErrorResponse} for c in (400, 403, 404, 409, 422, 500, 502)})
def run(req: DescribeRequest) -> ActionAccepted | JSONResponse:
    try:
        return execute(req)
