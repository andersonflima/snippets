...
[[ -n "${REPO_URL}" ]] || die "--repo-url é obrigatório"
...
resolve_instance_from_aws() {
  ...
  if [[ "${line_count}" -gt 1 ]]; then
    printf '[git-checkout-via-ec2] múltiplas instâncias encontradas para Name=%s:\n%s\n' "${INSTANCE_NAME}" "${output}" >&2
...
ensure_s3_bucket() {
  ...
  if [[ "${AWS_REGION_NAME}" == "us-east-1" ]]; then
    ...
    ${AWS_CMD[@]} s3api create-bucket --bucket "${S3_BUCKET}" --create-bucket-configuration "${bucket_region}" >/dev/null
  fi
}
...
poll_ssm_command() {
  ...
  while true; do
    status="$(${AWS_CMD[@]} ssm get-command-invocation --command-id "${command_id}" --instance-id "${INSTANCE_ID}" --query 'Status' --output text 2>/dev/null || true)"
    case "${status}" in
      Pending|InProgress|Delayed) sleep 3 ;;
      Success) return 0 ;;
      Cancelled|TimedOut|Failed|Cancelling) return 1 ;;
      *) sleep 2 ;;
    esac
  done
}
...
show_ssm_command_output() {
  ...
  command_id="$1"
  stdout_text="$(${AWS_CMD[@]} ssm get-command-invocation --command-id "${command_id}" --instance-id "${INSTANCE_ID}" --query 'StandardOutputContent' --output text 2>/dev/null || true)"
  stderr_text="$(${AWS_CMD[@]} ssm get-command-invocation --command-id "${command_id}" --instance-id "${INSTANCE_ID}" --query 'StandardErrorContent' --output text 2>/dev/null || true)"
...
resolve_remote_principal_arn_via_ssm() {
  ...
  parameter_file="$(make_temp_file "git-checkout-via-ec2-identity" ".json")"
...
PY
  ...
  command_id="$(${AWS_CMD[@]} ssm send-command --instance-ids "${INSTANCE_ID}" --document-name 'AWS-RunShellScript' --comment "git-checkout-via-ec2 identity probe ${INSTANCE_ID}" --parameters "file://${parameter_file}" --query 'Command.CommandId' --output text)"
  rm -f "${parameter_file}"
  ...
  principal_arn="$(${AWS_CMD[@]} ssm get-command-invocation --command-id "${command_id}" --instance-id "${INSTANCE_ID}" --query 'StandardOutputContent' --output text 2>/dev/null || true)"
...
ensure_s3_bucket_policy_for_remote_principal() {
  local current_policy_file merged_policy_file current_policy prefix_root object_resource bucket_resource sid_bucket sid_object normalized_principal
  resolve_remote_principal_arn_via_ssm
  normalized_principal="$(normalize_principal_arn "${REMOTE_PRINCIPAL_ARN}")"
  current_policy_file="$(make_temp_file "git-checkout-via-ec2-policy-current" ".json")"
  merged_policy_file="$(make_temp_file "git-checkout-via-ec2-policy-merged" ".json")"

  current_policy="$(${S3_AWS_CMD[@]} s3api get-bucket-policy --bucket "${S3_BUCKET}" --query 'Policy' --output text 2>/dev/null || true)"
  ...
  sid_bucket="GitCheckoutViaEc2ListBucket${INSTANCE_ID//-/}"
  sid_object="GitCheckoutViaEc2ObjectAccess${INSTANCE_ID//-/}"
...
commands.append(f'mkdir -p {shlex.quote(remote_dir)} {shlex.quote(remote_home_dir)}')
commands.append(f'export HOME={shlex.quote(remote_home_dir)}')
clone_cmd = ['git', '-c', 'http.version=HTTP/1.1', '-c', 'credential.helper=']
if insecure:
    clone_cmd.extend(['-c', 'http.sslVerify=false'])
clone_cmd.extend(['clone', '--no-checkout', repo_url, remote_repo_dir])
commands.append(' '.join(shlex.quote(part) for part in clone_cmd))
commands.append(f'git -C {shlex.quote(remote_repo_dir)} fetch --force --tags origin >/dev/null 2>&1 || true')
checkout_cmd = ['git', '-c', 'http.version=HTTP/1.1', '-c', 'credential.helper=']
if insecure:
    checkout_cmd.extend(['-c', 'http.sslVerify=false'])
...
run_remote_checkout() {
  ...
  parameter_file="$(make_temp_file "git-checkout-via-ec2-params-${RUN_ID}" ".json")"
  build_ssm_parameters_file "${parameter_file}"
  command_id="$(${AWS_CMD[@]} ssm send-command --instance-ids "${INSTANCE_ID}" --document-name 'AWS-RunShellScript' --comment "git-checkout-via-ec2 ${RUN_ID}" --parameters "file://${parameter_file}" --query 'Command.CommandId' --output text)"
  rm -f "${parameter_file}"
