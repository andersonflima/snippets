...
upload_input_archive_to_s3() {
  "${AWS_CMD[@]}" s3 cp "${INPUT_ARCHIVE_PATH}" "s3://${S3_BUCKET}/${INPUT_KEY}" --only-show-errors >/dev/null
}
...
download_result_from_s3() {
  "${AWS_CMD[@]}" s3 cp "s3://${S3_BUCKET}/${OUTPUT_KEY}" "${OUTPUT_ARCHIVE_PATH}" --only-show-errors >/dev/null
}
