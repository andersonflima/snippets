...
download_result_from_s3() {
  ...
  get_error_file="$(make_temp_file "git-fetch-via-ec2-s3-get" ".log")"

  if "${S3_AWS_CMD[@]}" s3 cp "s3://${S3_BUCKET}/${OUTPUT_KEY}" "${OUTPUT_ARCHIVE_PATH}" --only-show-errors >/dev/null 2>"${get_error_file}"; then
    rm -f "${get_error_file}"
    return 0
  fi
  ...
  if proxy_env_is_set && s3_signature_error_looks_like_proxy_issue "${get_error_file}"; then
    ...
    if env -u HTTPS_PROXY -u https_proxy -u HTTP_PROXY -u http_proxy -u ALL_PROXY -u all_proxy \
      "${S3_AWS_CMD[@]}" s3 cp "s3://${S3_BUCKET}/${OUTPUT_KEY}" "${OUTPUT_ARCHIVE_PATH}" --only-show-errors >/dev/null 2>"${get_error_file}"; then
      rm -f "${get_error_file}"
      return 0
    fi
  fi
  ...
  rm -f "${get_error_file}"
  die "falha ao baixar artefato do S3 para ${OUTPUT_ARCHIVE_PATH}"
}
