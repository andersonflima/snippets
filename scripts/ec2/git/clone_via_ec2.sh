...
Observação:
  Por padrão, o clone remoto não materializa objetos Git LFS no EC2.
...
Opções:
  ...
  --git-arg <valor>            Argumento adicional repassado para git clone no EC2. Repetível.
...
resolve_instance_from_aws() {
  ...
  if [[ "${line_count}" -gt 1 ]]; then
    printf '[git-clone-via-ec2] múltiplas instâncias encontradas para Name=%s:\n%s\n' "${INSTANCE_NAME}" "${output}" >&2
...
resolve_remote_principal_arn_via_ssm() {
  ...
  parameter_file="$(make_temp_file "git-clone-via-ec2-identity" ".json")"
...
PY
  ...
  command_id="$("${AWS_CMD[@]}" ssm send-command \
    ...
    --comment "git-clone-via-ec2 identity probe ${INSTANCE_ID}" \
...
ensure_s3_bucket_policy_for_remote_principal() {
  ...
  current_policy_file="$(make_temp_file "git-clone-via-ec2-policy-current" ".json")"
  merged_policy_file="$(make_temp_file "git-clone-via-ec2-policy-merged" ".json")"
  ...
  sid_bucket="GitCloneViaEc2ListBucket${INSTANCE_ID//-/}"
  sid_object="GitCloneViaEc2ObjectAccess${INSTANCE_ID//-/}"
...
remote_dir = f"/tmp/git-clone-via-ec2/{run_id}"
...
clone_cmd.extend(git_args)
clone_cmd.extend([repo_url, remote_repo_dir])

commands.append(" ".join(shlex.quote(part) for part in clone_cmd))
commands.append(f'test -d {shlex.quote(remote_repo_dir)}')
if materialize_lfs:
    lfs_guard = (
    ...
    commands.append(
        ...
        '{ echo "git lfs não está disponível no EC2 para materializar o clone" >&2; exit 1; }; '
        'git -C '
...
download_result_from_s3() {
  ...
  get_error_file="$(make_temp_file "git-clone-via-ec2-s3-get" ".log")"
...
run_remote_clone() {
  ...
  parameter_file="$(make_temp_file "git-clone-via-ec2-params-${RUN_ID}" ".json")"
  build_ssm_parameters_file "${parameter_file}"

  command_id="$("${AWS_CMD[@]}" ssm send-command \
    --instance-ids "${INSTANCE_ID}" \
    --document-name 'AWS-RunShellScript' \
    --comment "git-clone-via-ec2 ${RUN_ID}" \
    --parameters "file://${parameter_file}" \
    --query 'Command.CommandId' \
    --output text)"
